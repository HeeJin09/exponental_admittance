...
for i in range(N+3000):
	...
	# #################################################
	# dV_ref =lr.Adjoint(X_err) @ (dV_des - lr.dexp6(-lambda_) @ ddlambda_ref + lr.ad(V_err) @ V_des - lr.ddexp6(-lambda_, dlambda) @ dlambda)
	...
	pre_q_com = q_com 
	############# Position Control ###################
	p.setJointMotorControlArray(robotId, joint_list, p.POSITION_CONTROL ,targetPositions = q_com, forces=np.array([800,800,800,800,800,800])) 
